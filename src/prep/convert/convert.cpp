...
int main() {
    ...
    if (!ifile.is_open()) {
        std::cerr << "Error: could not open: " << wiresharkFile << std::endl;
        return 1;
    }
    ...
    if (!ofileXor.is_open()) {
        std::cerr << "Error: could not open: " << xorFile << std::endl;
        return 1;
    }

    if (!ofileErrDesc.is_open()) {
        std::cerr << "Error: could not open: " << errDescFile << std::endl;
        return 1;
    }
    ...
    while (getline(ifile, wsline)) {
        if (wsline[0] == '|' && wsline[1] == '0') {
            ...
            if (type == ETH2_TYPE_IP4) {
                ...
                if (ofile.fail()) {
                    std::cerr << "Error: writing to file: " << dataFile << " failed on frame " << info[3] << std::endl;
                    return 1;
                }
                ...
                if (ofileXor.fail()) {
                    std::cerr << "Error: writing to file: " << xorFile << " failed on xor-frame " << info[3] << std::endl;
                    return 1;
                }
                ...
                if (ofileErrDesc.fail()) {
                    std::cerr << "Error: writing to file: " << errDescFile << " failed on the desc of frame " << info[3] << std::endl;
                    return 1;
                }
